...
OUT = Path(r"C:\Users\Windows\Desktop\基于基因的眼病慢病诊断应用需求规格说明.docx")
...
hr = header.add_run('基于基因的眼病/慢病诊断应用｜需求规格说明')
...
add_table(doc, ['文档属性','内容'], [
    ...
    ('材料依据','工作区原型文件：基于基因的眼病_慢病诊断应用.html；结合 requirement-refiner skill 梳理'),
    ('文档状态','需求基线草案，供产品、临床、研发、算法、测试和数据安全评审'),
...
add_para(doc, '本次需求以现有原型已呈现的三步链路为基线：输入信息、智能分析、评估报告。原型中的“评分”“病灶标注”“方向性建议”和“导出”部分存在演示性质或占位实现，生产化规则必须经过临床、算法和合规评审后才能作为正式能力上线。')
...
add_bullets(doc, [
    ...
    '三步流程：输入信息、智能分析、评估报告。',
    '报告模块：患者背景、关键疾病基因易感性评分、影像异常提示与特征说明、基因变异解读、影像与基因综合疾病风险评估、方向性建议。',
    '报告导出入口：PDF、Word；本期需完成真实导出或明确为后续版本，不得仅以演示 toast 代替生产功能。',
])
...
add_table(doc, ['优先级','待确认项','影响'], [
    ...
    ('P1','PDF/Word 的模板、签名、机构信息和报告编号规则','影响导出实现及临床使用'),
...
add_para(doc, '原型采用一个多选下拉集合，将所有阳性/异常项目统一展示；勾选表示检测到阳性或异常，未勾选项目按“未检测/阴性”处理。生产系统不得继续混用这两个语义：至少应在数据模型中区分“未提供、未检测、阴性、阳性、异常、质量不合格”，并在报告中按真实状态表述。')
add_table(doc, ['分组','原型覆盖项目','输入规则','证据/互斥规则'], [
    ...
    ('脉络膜黑色素瘤驱动突变','GNAQ、GNA11、CYSLTR2、PLCB4、未检出驱动突变','驱动突变选项互斥；驱动事件不作为预后指标','需允许未测/不确定状态，避免把未输入误判为未检出'),
    ('预后相关标志物','BAP1 阳性/阴性；SF3B1 突变/野生型；EIF1AX 突变/野生型；染色体3单体/二体','同一标志物的相反状态互斥；未测应单独表达','检测到冲突组合时给出复核提示，不强行输出确定性结论'),
], widths=[1.45,2.55,2.0,1.95], font_size=8.1)
add_heading('7.4 眼底影像上传', level=2)
add_table(doc, ['字段/操作','规则','错误与反馈'], [
    ('影像类型','原型支持眼底彩照 CFP、OCT、OCTA；一次配置一种类型','类型变更后同步影像槽位；若原影像不再适用，需提示清理或重新上传'),
    ('上传眼数','原型支持单眼或双眼；双眼生成右眼/左眼槽位','双眼配置下不能只把一张图静默映射到双眼；允许缺一只眼的规则待确认'),
    ('本地图像上传','接受 image/*；生产需定义格式、大小、分辨率、方向和去元数据规则','格式不支持、读取失败、过大、病毒扫描失败时阻止进入分析并给出可恢复提示'),
    ('预览与清除','上传后展示缩略图/预览，支持清除后重新上传','清除需二次确认与状态更新；不得继续引用已清除文件'),
    ('最低条件','点击开始分析时至少有一张有效影像','未满足时提示“请至少提供一种眼底影像”，不创建分析任务'),
], widths=[1.55,3.55,2.85], font_size=8.7)

# 8 analysis
doc.add_heading('8 模块需求 智能分析', level=1)
add_para(doc, '智能分析页是输入与报告之间的任务状态页。原型展示多模态分析流水线和四个步骤：基因与影像联合分析、关键疾病特征提取、基因与病例证据关联、综合风险评估与报告。生产版必须将动画与真实任务状态绑定，不能只按定时器推进。')
add_table(doc, ['步骤','系统行为','可观察输出','失败处理'], [
    ...
    ('4 综合风险评估与报告','融合影像、基因和临床上下文，输出结构化风险及解释','生成报告 ID和版本','任一关键模块失败时按产品规则生成失败或部分结果报告，并显式标注缺失'),
...
add_table(doc, ['状态','进入条件','页面行为','可转移状态'], [
    ...
    ('处理中','至少一个分析步骤执行中','实时或轮询更新步骤；禁止重复创建相同任务','生成报告/失败/超时/取消'),
...
add_table(doc, ['区块','必须展示内容','可追溯信息','异常/缺失展示'], [
    ...
    ('基因变异解读','基因/标志物状态、证据强度、适用疾病方向、来源/限制','知识条目 ID、版本、引用来源','未测、阴性、质量不合格不得混为一类'),
    ('影像与基因综合疾病风险评估','疾病、风险等级、构成因素、主要证据和限制','风险规则版本、输入摘要','冲突组合或证据不足应输出复核提示'),
...
add_bullets(doc, [
    ...
    '对于基因与影像方向不一致、BAP1/染色体3等标志物出现冲突、或数据质量不足的情况，报告应显示“需专业复核”，不按最差单项自动判为最高风险。',
    '建议文案必须由临床专家审核；示例中的 3–6 个月、6–12 个月、年度筛查等周期不能未经确认直接固化为通用医嘱。',
])
add_heading('9.3 报告导出', level=2)
add_table(doc, ['格式','内容要求','文件命名/版本','验收重点'], [
    ('PDF','与页面报告一致，包含免责声明、报告编号、生成时间、版本信息和页眉页脚','命名规则待确认；导出必须绑定报告版本','中文字体、表格分页、影像缩略图、敏感信息控制'),
    ('Word','可编辑的结构化报告，至少保留标题、表格、风险解释和免责声明','命名规则待确认；导出后不得改变报告数据','内容完整、格式可读、无演示 toast、不可导出未授权字段'),
], widths=[0.85,3.6,1.85,2.0], font_size=8.5)
...
add_table(doc, ['接口','请求业务输入','成功输出','失败输出'], [
    ...
    ('导出报告','report_id、格式、字段范围、访问者身份','文件任务或文件引用、导出状态','格式不支持、内容超限、无权限、生成失败'),
...
add_table(doc, ['场景','系统行为','用户可见反馈','验收要求'], [
    ...
    ('导出失败','保留报告本身，导出任务可重试','显示失败原因，不生成损坏文件','失败文件不可下载'),
...
add_table(doc, ['编号','前置条件','操作','预期结果'], [
    ...
    ('AC-11','报告成功生成','导出PDF和Word','生成与当前报告版本一致的文件，包含免责声明、版本和必要的审计标识'),
    ('AC-12','用户重复点击开始分析','短时间内连续点击两次','只创建一个可识别任务，页面给出处理中/已提交反馈'),
    ('AC-13','点击重新分析','从报告返回输入页','不得静默覆盖原报告；新任务或新版本可追溯到原任务'),
], widths=[0.7,2.1,2.8,3.0], font_size=8.1)

# 14 assumptions and change log
doc.add_heading('14 工作假设与待产品确认汇总', level=1)
add_table(doc, ['编号','事项','本文暂采用的工作假设','需要谁确认'], [
    ...
    ('Q-06','报告导出','PDF/Word为真实生产能力，而非原型提示；模板和签名规则待确定','产品、研发、合规'),
    ('Q-07','数据治理','敏感数据采用最小权限、审计、脱敏和生命周期管理','安全、运维、合规'),
...
add_bullets(doc, [
    ...
    '已覆盖患者信息、基因/标志物、眼底影像、任务、报告和导出等核心对象。',
    '已区分原型事实、工作假设和待产品确认项；未将演示数据、示意分数和定时动画直接固化为临床规则。',
    '已覆盖角色权限、隐私安全、异常恢复、幂等、版本追溯和报告免责声明。',
    '已给出可观察的验收用例，包含正常、异常、权限和状态路径。',
])
add_callout(doc, '上线前阻塞项', '在正式上线前，必须完成疾病与基因知识库审核、影像模型临床验证、风险分层规则审批、数据安全与隐私评审，以及 PDF/Word 报告模板确认。', fill='F4CCCC', color='990000')
...
props.author = 'Codex'
props.comments = '基于现有 HTML 原型和 requirement-refiner skill 生成'
doc.save(OUT)
print(str(OUT))
